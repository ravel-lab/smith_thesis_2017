...
-Sctaych area vs EdU
-Drop RF figure
-Choose only one filtering metric for RF
...
LACTO_PROP<-rowSums(dplyr::select(SRL_16S,c(Lactobacillus_crispatus,Lactobacillus_jensenii,Lactobacillus_iners,Lactobacillus_gasseri)))
##LACTO_PROP<-rowSums(dplyr::select(SRL_16S,c(Gardnerella_vaginalis)))
...
##model_input$LACTO_PROP<-as.character(model_input$LACTO_PROP)
...
model_input_Lactobacillus<-dplyr::select(model_input[!is.na(model_input$LACTO_PROP),],-c(NUGENT_SCORE,CST))## Keep CSTs in model_input, just not Amsel or Nugent input
#model_input_Nugent<-dplyr::select(model_input[!is.na(model_input$NUGENT_SCORE),],-c(LACTO_PROP,CST)) ## Keep CSTs in model_input, just not Amsel or Nugent input
...
Lacto_RF$rfp$importance_w_pval
...
TABLE_RF_SUMMARY_output<-data.frame(features=names(model_input %>% dplyr::select(-c(LACTO_PROP,NUGENT_SCORE,SID))))
...
load("/Volumes/HD-PATU3/Jacques_Steve_Shared/Manuscript/miRNA/Submission/Supplementary_Data/Analysis_Pipeline/Script_input/qPCR_time_course.Rdata")
load("/Volumes/HD-PATU3/Jacques_Steve_Shared/Manuscript/miRNA/Submission/Supplementary_Data/Analysis_Pipeline/Script_input/qpcr_sigtest.Rdata")
...
write.csv(qPCR_time_course,"~/all_qpcr.csv",quote = F,row.names = F)
ggplot()+geom_point(data=dplyr::filter(qPCR_time_course,BCS %in% c("L. crispatus","L. jensenii","L. iners", "G. vaginalis","Cell Culture Medium") & Study %in% c("study_6", "study_5")),aes(x=ExposureTime.n,y=miR193b_Ct,col=BCS,pch=Study))#+
#  geom_jitter(data=dplyr::filter(qPCR_time_course,BCS %in% c("L. crispatus","L. jensenii","L. iners", "G. vaginalis","Cell Culture Medium")),aes(x=ExposureTime.n,y=miR193b_Ct,col=BCS))+facet_wrap(~Study)
...
# 
## sd delta Ct =deltaCt.RNU6.sd
dev.off()


#qpcr_sigtest<-rbind.fill(qpcr_sigtest,
#data.frame(reference="Cell Culture Medium",
 #          ExposureTime=c(rep(c("8hr","16hr","19hr"),each=2)),
  #                 #  ExposureTime.n=c(rep(c(8,16,19),each=3)),
   #        comparison=rep(c("L. jensenii","L. iners"),times=3),
          
           #ref="Cell Culture Medium",
           
    #       Study.ref="study_6",Study.comp="study_6"
     #      ))
...
PLOT_QPCR_TIMECOURSE<-
 # ggplot(filter(qpcr_sigtest,reference=="G. vaginalis" &
   ggplot(filter(qpcr_sigtest,reference=="Cell Culture Medium" &
                   comp %in% c("L. crispatus","L. jensenii","L. iners")&
  !grepl(qpcr_sigtest$comparison,pattern = "_10|_766|0_")), #& qpcr_sigtest$reference== "G. vaginalis"),
...
scratch.summary$ExposureTime<-et
...
ggplot(scratch.summary.all)+geom_line(aes(x=as.numeric(gsub(scratch.summary.all$ExposureTime,pattern = "hr",replacement = "")),y=mean,col=BCS),show.legend = F)+
  geom_errorbar(aes(x=as.numeric(gsub(scratch.summary.all$ExposureTime,pattern = "hr",replacement = "")),ymin=mean-sd,ymax=mean+sd),width=.3,show.legend = F)+
  mBio+
