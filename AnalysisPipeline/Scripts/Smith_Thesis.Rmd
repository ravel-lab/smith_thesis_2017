...
selected_metadata<-dplyr::select(pData(features_metadata),c(SID,NUGENT_SCORE,AMSEL_prediction,Pre_QC_ID,CST)) 
...
model_input$AMSEL_prediction<-as.numeric(model_input$AMSEL_prediction)
...
Lacto_RF<-run_randomForest(predictors_response_table = model_input_Amsel,
...
Lacto_RF$importance[order(Lacto_RF$importance$X.IncMSE,decreasing = T),]
