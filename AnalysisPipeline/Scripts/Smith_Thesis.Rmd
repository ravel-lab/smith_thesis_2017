...
proxy_amsel_predictions<-rowSums(dplyr::select(SRL_16S,c(Lactobacillus_crispatus,Lactobacillus_jensenii,Lactobacillus_iners,Lactobacillus_gasseri)))
##proxy_amsel_predictions<-rowSums(dplyr::select(SRL_16S,c(Gardnerella_vaginalis)))
...
Lacto_RF$importance[order(Lacto_RF$importance$X.IncMSE,decreasing = F),]
(Lacto_RF$top_features$top_features.all<-row.names(Lacto_RF$importance[Lacto_RF$importance$X.IncMSE>=2,]))
...
model_input.out<-model_input[,names(model_input) %in% unique(c(names(model_input_Amsel)))]

predictor_names<-names(dplyr::select(model_input.out,-c(AMSEL_prediction,SID)))
...
TABLE_RF_SUMMARY_output<-data.frame(features=row.names(Lacto_RF$rfp$importance_w_pval),AMSEL=Lacto_RF$rfp$importance_w_pval)
...
topmir_plot_data<-model_input[!is.na(model_input$NUGENT_SCORE),names(model_input) %in% c("NUGENT_SCORE","AMSEL_prediction","CST",as.character(top_mirs_table$miRNA))]
...
PLOT_EXPRESSION<-
  ggplot(topmir_plot_data,aes(x=AMSEL_prediction,y=value))+
...
Nugent_RF.Boruta<-Boruta(x=dplyr::select(model_input_Amsel,-c(SID)),y=model_input_Amsel$AMSEL_prediction)
