...
FIGURE_WESTERN_QUANT<-"Figure_4_H.eps"
...
setup_sigtest(pval_threshold = pval_threshold,raw_data = dplyr::filter(control_meds,ExposureTime==et,Observation=="Proliferation"),test_function = "t.test",Experiment = "Scratch")
...
PLOT_nyc_v_tsb<-ggplot(control_meds.summary)+
  geom_bar(aes(x=Treatment,y=mean,fill=Treatment),stat="identity",show.legend =F)+
  geom_errorbar(aes(x=Treatment,ymax=mean+sd,ymin=mean-sd),width=.2)+
  mBio+ylab("Percent scratch area filled | Percent cells EdU")+
...
Western<-read.csv(paste0(root_directory,"../PreparePipeline/western_summary.csv"))
Western$BCS<-factor(Western$BCS,levels = c("L. crispatus","L. jensenii","L. iners","G. vaginalis","Cell Culture Medium","0.1% DL lactic acid"),ordered = T)

western.plot<-ggplot(dplyr::filter(Western,!ExposureTime==24),aes(x=as.factor(ExposureTime),y=100*BCS_MEDIA,fill=BCS))+geom_bar(stat = "identity",width=.75, position = "dodge")+
...
cairo_ps(file=paste0(thesis_figures_directory,FIGURE_WESTERN_QUANT),width = 8,height = 5.5)
plot(western.plot)
dev.off()
