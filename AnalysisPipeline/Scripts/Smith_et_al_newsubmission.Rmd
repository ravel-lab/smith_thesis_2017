...
---
# Analysis Script
**The following markdown serves as a record for partial analysis performed in Edwards, Smith et al., 2018.**   
...
TABLE_SEQSUMMARY<-"DROP_TABLE_1.csv"
#TABLE_TOPMIRS<-"TABLE_2.txt"
TABLE_GO_GENES<-"DROP_TABLE_2.txt"
TABLE_TRL_NUMDEGENES<-"DROP_TABLE_3.csv"
TABLE_TRL_SUMMARY_PATHWAYS<-"DROP_TABLE_4.csv"

TABLE_COUNTS_RAW<-"SS_TABLE_S1.csv"
TABLE_MODEL_INPUT<-"SS_TABLE_S2.csv"
TABLE_SRL_METADATA<-"SS_TABLE_S3.csv"
TABLE_RF_SUMMARY<-"SS_TABLE_S4.csv"
TABLE_MIR_TARGETS<-"DROP_XTABLE_S5.txt" # List of gene targets for miR-193b, others; contains commas so needs to be txt
TABLE_QPCR_TIMECOURSE<-"DROP_XTABLE_S6.csv"
TABLE_EDU_SCRATCH_QUANT<-"DROP_XTABLE_S7.csv"
TABLE_WESTERN_QUANT<-"DROP_XTABLE_S8.csv"
TABLE_CT_QUANT<-"DROP_XTABLE_S9.csv"
TABLE_TRL_ALIGNSTATS<-"SS_TABLE_S10.csv"
TABLE_TRL_COUNTS_RAW<-"SS_TABLE_S11.csv"
TABLE_EDGER_RESULTS<-"SS_TABLE_S12.csv"
TABLE_TRL_PATHWAY_Z_SCORES<-"SS_TABLE_S13.csv"

FIGURE_SUBJECT_PLOTS<-"DROP_FIGURE_1_"
FIGURE_RF_IMPORTANCE_AMSEL<-"DROP_FIGURE_2.eps"
FIGURE_RF_IMPORTANCE<-"DROP_Figure_drop.eps"
FIGURE_TOPMIRS<-"FIGURE_4A_and_FIGURE_S7C.eps"
FIGURE_QPCR_TIMECOURSE<-"FIGURE_S8A.eps" 
FIGURE_EDU_QUANT<-"FIGURE_5B.ps"
FIGURE_SCRATCH_QUANT<-"FIGURE_5A.ps"
FIGURE_CT_INFECT_QUANT<-"FIGURE_6B.ps"
FIGURE_CT_EDU_QUANT<-"FIGURE_6A.ps"
FIGURE_WESTERN_QUANT<-"FIGURE_S8B.ps"
FIGURE_WESTERN_TRANSFECTION_QUANT<-"FIGURE_S8B_2.ps"
FIGURE_MIR193_TARGETS_GROUP<-"FIGURE_4B.ps"
FIGURE_MIR193_TARGETS_GO<-"FIGURE_4C.ps"

FIGURE_COMBINED_PATHWAYS_CYCLE<-"DROP_Figure_7.eps"
FIGURE_LONGITDUINAL_GENEEXP.cycle<-"FIGURE_7.eps"
FIGURE_FINAL_MODEL<-"DROP_Figure_9.eps" ##Placeholder

FIGURE_QC<-"FIGURE_S6"
FIGURE_QC_RIN_v_READS.PROP<-paste0("DROP_",FIGURE_QC,"1.eps")
FIGURE_QC_RIN_v_READS.ABS<-paste0("DROP_",FIGURE_QC,"2.eps")
FIGURE_QC_PCA.PREQC.BYBATCH<-paste0(FIGURE_QC,"A.eps")
FIGURE_QC_PCA.PREQC.BYSUBJ<-paste0(FIGURE_QC,"B.eps")
FIGURE_QC_PCA.RMLOW.BYBATCH<-paste0(FIGURE_QC,"C.eps")
FIGURE_QC_PCA.RMLOW.BYSUBJ<-paste0(FIGURE_QC,"D.eps")
FIGURE_QC_PCA.NORMAL.BYBATCH<-paste0(FIGURE_QC,"E.eps")
FIGURE_QC_PCA.NORMAL.BYSUBJ<-paste0(FIGURE_QC,"F.eps")
FIGURE_MIR_TARGETS_GO<-"FIGURE_S7B.eps"
FIGURE_DL_LACTICACID_QPCR<-"DROP_Figure_S3.eps" ## maybe add lactic acid time course here and split out 13h D, L, DL panel. 
##S4 =  EdU microscopy
FIGURE_nyc_v_tsb<-"DROP_Figure_S5.eps" ## Add EdU barplot as well. 
...
setup_sigtest<-function(pval_threshold = pval_threshold,raw_data = raw_data,test_function = "t.test",Experiment="Scratch"){
  ...
  if(Experiment=="Scratch"){
    sigtest<-data.frame(xref=c(rep("L. crispatus",times=5),rep("L. jensenii",times=4),rep("L. iners",times=3),rep("0.1% DL lactic acid",times=2),"G. vaginalis"),reference=c("L. jensenii", "L. iners","0.1% DL lactic acid","G. vaginalis","Cell Culture Medium",  "L. iners", "0.1% DL lactic acid","G. vaginalis", "Cell Culture Medium","0.1% DL lactic acid","G. vaginalis","Cell Culture Medium","G. vaginalis","Cell Culture Medium","Cell Culture Medium"))
    
    ## Factor labels
    sigtest$xref<-factor(sigtest$xref,levels =c("L. crispatus","L. jensenii","L. iners","G. vaginalis","Cell Culture Medium","0.1% DL lactic acid"),ordered = T)
    sigtest$reference<-factor(sigtest$reference,levels = c("L. jensenii","L. iners","G. vaginalis","Cell Culture Medium","0.1% DL lactic acid"),ordered = T)
  ...
  if(Experiment=="Scratch"){
    ...
    statbars[statbars$reference=="Cell Culture Medium","y"]<-rep(seq(100,88,-3),times=1)
    statbars[statbars$reference=="G. vaginalis","y"]<-rep(seq(88,by=-3,length.out = 4),times=1)
...
print(paste0("The median RINe score was ",median_RIN,"."))
...
## Make note of which samples aren't going into RF model because there's no Lacto info
samples_without_16S<-setdiff(row.names(model_input),row.names(model_input_Lactobacillus))
(removed_samples<-remove_poorQC_samples(removed_samples =removed_samples, 
                                        sample_list =samples_without_16S,
                                        reason = "No metataxonomic data"))

#SRL_meta_table[SRL_meta_table$Pre_QC_ID %in% samples_without_16S,"QC_removal_stage"]<-"No metataxonomic data"

##ADD QC STAGE TO SRL_meta_table, then write to disk
SRL_meta_table<-left_join(SRL_meta_table,removed_samples,by="Pre_QC_ID")
print(paste0("There were 5 samples removed due to insufficient library material or failure to sequence, ",sum(!is.na(SRL_meta_table$QC_removal_stage) & SRL_meta_table$QC_removal_stage %in% c("Low_Coverage","Visual Outlier"))," samples were removed due to low total miRNA reads or outliers and ",sum(!is.na(SRL_meta_table$QC_removal_stage) & SRL_meta_table$QC_removal_stage %in% c("No metataxonomic data"))," samples for which no metataxonomic data exists."))
...
SRL_seq_summary<-ddply(SRL_meta_table,c("BVGroup","SID"),summarise,
                       PreQC=length(QC_removal_stage),
                       PostQC=length(QC_removal_stage)-sum(as.numeric(QC_removal_stage %in% c("Low_Coverage","Visual Outlier")),na.rm = T),
                       With_metataxonomic=sum(as.numeric(is.na(QC_removal_stage)),na.rm = T)
                       )
...
names(model_input_Lactobacillus)
...
(group_freq<-group_freq[order(group_freq$Freq,decreasing = T),])
...
miR_order<-unique(dplyr::select(topmir_plot_data,c(variable,adj.r.squared)))
miR_order<-miR_order[order(miR_order$adj.r.squared,decreasing = T),]
mir_go.wide.filtered.m$mir<-factor(mir_go.wide.filtered.m$mir,levels = miR_order$variable,ordered = T)

# since only plotting miR-193b, plot counts, not percentage
scalar<-unique(mir_go.counts[mir_go.counts$mir=="miR-193b","number_targets"])#100

PLOT_MIR_TARGET_GO_PROP<-ggplot(mir_go.wide.filtered.m,aes(x=GO_format,y=scalar*value,fill=mir))+geom_bar(width=.7,position = "dodge",col="grey",stat="identity")+
  journalFormat+
...
## for miR-193b, list genes involved in each process and write as table
mir_go.mir193b<-dplyr::filter(mir_go,mir=="miR-193b") ## filter on mir193b
mir_go_genes.mir193b<-data.frame(GO=unique(mir_go.mir193b$GO))
## FOr each GO term, make a comma separated list of genes in GO
for(GO in mir_go_genes.mir193b$GO){
  mir_go_genes.mir193b[mir_go_genes.mir193b$GO==GO,"gene"] <- str_c(mir_go.mir193b[mir_go.mir193b$GO==GO,"gene"],collapse = ", ")
}
## sort based on most to least genes in GO
mir_go_genes.mir193b<-mir_go_genes.mir193b[order(str_count(string = mir_go_genes.mir193b$gene,pattern = ", "),decreasing = T),]

## Filter on GO terms with at least 2 genes
mir_go_genes.mir193b<-mir_go_genes.mir193b[str_count(string = mir_go_genes.mir193b$gene,pattern = ", ")>=2,]

## The following filters just on what is reported in figure... drops some GO terms not shared with top miRNAs. 
##FIXME MAke sure to metnion this in manuscript
mir_go_genes.mir193b[mir_go_genes.mir193b$GO %in% mir_go.wide.filtered.m.193b$variable,]

## Not in GO figure:
mir_go_genes.mir193b[!mir_go_genes.mir193b$GO %in% mir_go.wide.filtered.m.193b$variable,]

write.table(mir_go_genes.mir193b,file = paste0(thesis_tables_directory,TABLE_GO_GENES),sep="\t",quote=F,row.names = F)
...
# old plot
ggplot(mir193b_GOcount_groups,aes(x=Var1,y=Freq))+geom_bar(width=.7,position = "dodge",col="grey",stat="identity")+
  journalFormat+
...
# Generate miR-193b only plots
mir_go.wide.filtered.m.193b
mir_go.mir193b
#HACK FIX! This is read in twice, because upstream it gets filtered
group_go_map<-read.csv(file=paste0(R_script_input_directory,"GO_Process_Grouping_Map.csv"))
mir_go_group.mir193b<-merge(mir_go.mir193b,group_go_map,by.x="GO",by.y = "variable")
intersect(mir_go.mir193b$GO,group_go_map$variable)
mir_go.mir193b[mir_go.mir193b$GO=="GO:0060749~mammary gland alveolus development",]
group_go_map[group_go_map$variable=="GO:0060749~mammary gland alveolus development",]
mir_group.mir193b.summary<- ddply(unique(dplyr::select(mir_go_group.mir193b,c("Group","gene"))),c("Group"),summarise,n=length(gene))

mir_go.mir193b.summary<-ddply(unique(dplyr::select(mir_go_group.mir193b,c("Go_format","Group","gene"))),c("Go_format","Group"),summarise,n=length(gene))

mir_go.mir193b.summary$Group<-factor(mir_go.mir193b.summary$Group,levels =mir_group.mir193b.summary[order(mir_group.mir193b.summary$n,decreasing = T),"Group"] ,ordered = T)

mir_group.mir193b.summary$Group<-factor(mir_group.mir193b.summary$Group,levels =mir_group.mir193b.summary[order(mir_group.mir193b.summary$n,decreasing = T),"Group"] ,ordered = T)


number_miR193_targets<-unique(mir_go.counts[mir_go.counts$mir=="miR-193b","number_targets"]) # sloppy

mir193b_GOcount_groups.plot<-ggplot(mir_group.mir193b.summary,aes(x=Group,y=100*n/number_miR193_targets))+geom_bar(width=.7,position = "dodge",col="grey",stat="identity")+
  journalFormat+
  theme(axis.text.x = element_text(angle=45,hjust = 1),text=element_text(size=25),plot.margin = unit(c(10,25,10,170),units = "pt"))+
  scale_fill_brewer(type = "qual",palette = "Set1")+
  scale_y_continuous(limits = c(0,100),breaks=seq(0,100,by=10))+
  xlab("GO process")+
  ylab("Percent miR-193b targets")

cairo_ps(paste0(thesis_figures_directory,FIGURE_MIR193_TARGETS_GROUP),width = 11,height = 8.5)
plot(mir193b_GOcount_groups.plot)
dev.off()

mir_go.mir193b.summary$Go_format<-factor(mir_go.mir193b.summary$Go_format,levels = mir_go.mir193b.summary[with(mir_go.mir193b.summary, order(Group, -rank(n),decreasing = F)),"Go_format"],ordered = T)
mir_go.mir193b.summary.filter<-dplyr::filter(mir_go.mir193b.summary,n>=2)
mir193b_GOcount.plot<-ggplot(mir_go.mir193b.summary.filter,aes(x=Go_format,y=n,fill=Group))+geom_bar(width=.7,position = "dodge",col="grey",stat="identity")+
  journalFormat+
  theme(axis.text.x = element_text(angle=45,hjust = 1,size=10),text=element_text(size=12),plot.margin = unit(c(10,25,10,170),units = "pt"))+
  scale_fill_brewer(type = "qual",palette = "Set1")+
  xlab("GO Term")+
...
cairo_ps(paste0(thesis_figures_directory,FIGURE_MIR193_TARGETS_GO),width = 11,height = 8.5)
plot(mir193b_GOcount.plot)
dev.off()
## remove miR-193b from plot
## verify no targets for bigger plot
...
PLOT_QPCR_TIMECOURSE
dev.off()
##FIXME- this figure should have more reference--- what am I trying to say?
## Show D and L lactic acid qPCR results compared to other references
ddct_LacticAcid_plot<-
  ggplot(filter(qpcr_sigtest,ExposureTime=="4hr" & !qpcr_sigtest$reference=="0.06% D-lactic acid" & !reference %in% c("G. vaginalis")),aes(x=reference,y=-mean_diff,col=comparison))+
  theme_bw()+
  geom_hline(yintercept = 0,col="#9e9ac8",size=2.5)+
...
qpcr_sigtests[qpcr_sigtests$ExposureTime=="4hr" & !qpcr_sigtests$reference=="0.06% D-lactic acid" & !qpcr_sigtests$comparison %in% c("L. crispatus","L. jensenii","L. iners","G. vaginalis"),"Figure"]<-gsub(FIGURE_DL_LACTICACID_QPCR,pattern=".eps",replacement = "")
## Only plotting medium as reference, filter for table
qpcr_sigtests<-dplyr::filter(qpcr_sigtests,reference=="Cell Culture Medium")
...
scratch.summary.all$ExposureTime<-paste0(scratch.summary.all$ExposureTime.n,"hr")
# This was removed from plot!
scratch.summary.all<-merge(scratch.summary.all,dplyr::filter(scratch_sigtest.all,reference=="Cell Culture Medium"),by.x=c("BCS","ExposureTime"),by.y=c("xref","ExposureTime"),all.x=T)
...
PLOT_SCRATCH<-
  ...
  scale_x_continuous(breaks=c(1,4,8,13,16,19,22),minor_breaks = NULL,limits=c(0,22.5))+
   geom_text(aes(x=as.numeric(ExposureTime.n),y=mean,label=sig),size=8,col="black")+
...
# This was removed from plot!
EdU.summary<-merge(EdU.summary,dplyr::filter(EdU_sigtest,reference=="Cell Culture Medium"),by.x=c("BCS","ExposureTime"),by.y=c("xref","ExposureTime"),all.x=T)
...
PLOT_EDU<-
  ...
  scale_color_manual(values = color_scheme_BCS)+
  scale_x_continuous(breaks=c(1,4,8,13,16,19,22),minor_breaks = NULL,limits=c(0,22.5))+
    geom_text(aes(x=as.numeric(ExposureTime),y=mean,label=sig),size=8,col="black")+
...
proliferation_sigtests<-rbind(proliferation_sigtests,
                              data.frame(Figure="FIGURE_S5_B",
                                         ...
                                         reference="NYC",
                                         ExposureTime="13hr",
                              pval= control_meds.summary$p.value,
...
Western$ExposureTime<-paste0(Western$ExposureTime,"hr")
write.csv(Western,file=paste0(thesis_tables_directory,TABLE_WESTERN_QUANT),row.names = F)

Western_transfection<-read.table(paste0(R_script_input_directory,"Western_transfection.txt"),header = T,sep = "\t")
Western_transfection$Transfection<-factor(Western_transfection$Transfection,ordered = T,levels = c("miR-193b mimic","Scramble control","miR-193b mimic only","Scramble control only","L. jensenii BCS"))

western_transfection.plot<-ggplot(Western_transfection,aes(x=Transfection,y=CCND1_BACTIN,fill=Transfection))+
  geom_bar(stat = "identity",width=.75, position = "dodge")+
  scale_fill_manual(values=c("L. jensenii BCS"="#F9D825",
                            "miR-193b mimic"="#ae017e",
                            "miR-193b mimic only"="#f768a1",
                           "Scramble control"="#2171b5",
                           "Scramble control only"="#6baed6"))+
  journalFormat+
  theme(axis.text.x = element_text(angle = 35,hjust = 1))+
  ylab("CCND1/B-actin intensity")+
  xlab("Transfection type")

postscript(file=paste0(thesis_figures_directory,FIGURE_WESTERN_TRANSFECTION_QUANT),width = 8,height = 5.5)
plot(western_transfection.plot)
dev.off()
...
## Plot Infectivity
CT_INFECT_QUANT.plot<-ggplot(filter(pi.summary,Observation=="Infectivity"),aes(x=Treatment,y=grand_mean))+geom_bar(aes(fill=Treatment),stat="identity",show.legend = F)+geom_errorbar(aes(ymin=grand_mean-grand_sd,ymax=grand_mean+grand_sd,width=.3))+
...
## Combine proliferation sig tests and write to file
FIGURE_CT_EDU_QUANT
ct_sigtests<-data.frame(Figure=paste0(gsub(FIGURE_CT_EDU_QUANT,pattern = ".ps",replacement = ""),"_",ct_sigtest$Observation),ct_sigtest)
...
## Filter metadata table to only include samples that were extracted and passed sample QC. Be sure to include samples that were extracted but which no metadataxonmic data exists. 
miRNA_extractions<-SRL_meta_table[is.na(SRL_meta_table$QC_removal_stage ) | !SRL_meta_table$QC_removal_stage %in% c("No metataxonomic data"),]## Samples associated with miRNA libraries used in final study- i.e., don't include samples removed due to poor QC. Also adding in only samples that have 16S data as well. Will wind up dropping 16 samples, but it is what it is... 

subject_plot_list<- unique(miRNA_extractions$SID) ## Subject IDs to plot, including samples that may not have usable data in final RF model
...
for(s in subject_plot_list){  ## iterate through the subject's
  ...
  ## Determine the min & max of time points- these will be used to 'grey out' samples that were dropped because of 1) poor QC 2) extracted for smallRNA seq but not 16S
  ## The fina figure will have greyed out boxes if a sample was not used in RF models, but will have a line for the time point if data exists (i/e/, 16S data but not small RNAseq). 
  rect_min<-time_breaks[!time_breaks%in% miRNA_extractions[miRNA_extractions$SID==s &  is.na(miRNA_extractions$QC_removal_stage),"SERIAL"]]-.5
  rect_max<-time_breaks[!time_breaks%in% miRNA_extractions[miRNA_extractions$SID==s & is.na(miRNA_extractions$QC_removal_stage),"SERIAL"]]+.5
  ...
  ## Determine any dropped sample (post QC) time points and place an * above, but do not place an * for samples without metataxonomic (these are annotated with greyed boxes-- they weren't dropped because of poor QC. )
  dropped_samples.serial<-otu_count[otu_count$Pre_QC_ID %in% removed_samples[removed_samples$QC_removal_stage %in% c("Low_Coverage"),"Pre_QC_ID"],"SERIAL"]
...
write.csv(dplyr::select(summary_pathways,c(comparison,contains("cycle"))),paste0(thesis_tables_directory,TABLE_TRL_SUMMARY_PATHWAYS),row.names=F,quote=F)
...
## Clean up some of the pathway names in pathway map since the formatting gets screwed up
...
# 
# dev.off()
pathway_zscores.medium.melt.cycle$comp_label<-factor(pathway_zscores.medium.melt.cycle$comp,levels = c("LCM","LJM","LIM","GVM"),labels=c("L. crispatus BCS","L. jensenii BCS","L. iners BCS", "G. vaginalis BCS"),ordered = T)

cairo_ps(paste0(thesis_figures_directory,FIGURE_COMBINED_PATHWAYS_CYCLE),width=10,height=8)

ggplot(pathway_zscores.medium.melt.cycle)+geom_tile(aes(x=ExposureTime,y=pathway,fill=value))+
scale_fill_gradient2(guide = guide_colourbar(title="Z-score"),high="red",mid="white",low="blue", 
       na.value="yellow", midpoint=0)+
  facet_wrap(~comp_label,nrow=1)+
  journalFormat+
  theme(text = element_text(size=12),plot.margin = unit(c(0,30,0,0),units = "pt"))+
  xlab("Exposure time (hours)")+
  ylab("Cell cycle pathway")
...
## Only output comparisons used in manuscript
edgeR_results.subset<-dplyr::select(edgeR_results,c(gene,matches("[LC|LJ|LI|GV]M")))

write.csv(edgeR_results.subset,paste0(thesis_tables_directory,TABLE_EDGER_RESULTS),quote = F,row.names = F)
