...
#ggplot(read.table("~/test",header = T,sep = "\t"),aes(x=t,y=pct,col=bcs))+geom_line()
...
control_meds.summary<-ddply(control_meds,c("Observation","Treatment"),summarise,mean=mean(percent_cells),sd=sd(percent_cells))
...
control_meds.sig.edu<-t.test(x=filter(control_meds,Observation=="EdU" & Treatment=="CONTROL_NYCmed_20pct")[,"percent_cells"],y =filter(control_meds,Observation=="EdU" & Treatment=="CONTROL_TSBmed_20pct")[,"percent_cells"],paired = F)
control_meds.sig.scratch<-t.test(x=filter(control_meds,Observation=="Proliferation" & Treatment=="CONTROL_NYCmed_20pct")[,"percent_cells"],y =filter(control_meds,Observation=="Proliferation" & Treatment=="CONTROL_TSBmed_20pct")[,"percent_cells"],paired = F)

##FIXME
paste0("Percent cell proliferation between 20% NYC-III and TSB bacterial culture media as evaluated by scratch assay and EdU was not significant (mean +/- standard deviation NYC-III scratch area filled=",control_meds.summary)#control_meds.nyc.mean,"% +/- ",control_meds.nyc.sd,"%, mean +/- standard deviation TSB  cell proliferation=,",control_meds.tsb.mean,"% +/- ",control_meds.tsb.sd,"%, p=",control_meds.sig$p.value,")")
...
##FIXME - need an EdU and scratch. 
PLOT_nyc_v_tsb.scratch<-ggplot(data.frame(media=c("NYC (Lactobacillus spp.)","TSB (G. vaginalis)"),scratch.mean=c(
  dplyr::filter(control_meds.summary,Observation=="Proliferation",Treatment=="CONTROL_NYCmed_20pct")$mean,
  dplyr::filter(control_meds.summary,Observation=="Proliferation",Treatment=="CONTROL_TSBmed_20pct")$mean),
  scratch.sd=c(control_meds.nyc.sd,
               control_meds.tsb.sd)))+
