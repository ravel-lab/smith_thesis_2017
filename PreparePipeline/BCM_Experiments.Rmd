...
LCM$BCM<-factor(LCM$BCM,ordered = T,levels = c("LCRISP","LINERS","LJENS","GVAG","MEDIA","LACTICACID","NTC","0hr","CONTROL","FASC_INHIBITOR","SIS_INHIBITOR","CK4_INHIBITOR",paste0(rep(c("DL_","L_"),each=2),rep(c("0_08","0_06"),times=2)),paste0(rep(c("0_08_","0_06_"),times=3),rep(c("DL","L","D"),each=2)),LA_10_12,LA_ph),labels = c("L. crispatus","L. iners","L. jensenii","G. vaginalis","Cell Culture Medium","lactic acid","Negative Control","0hr","CONTROL","FASC_INHIBITOR","SIS_INHIBITOR","CK4_INHIBITOR",paste0(rep(c("DL_","L_"),each=2),rep(c("0_08","0_06"),times=2)),paste0(rep(c("0_08_","0_06_"),times=3),rep(c("DL","L","D"),each=2)),LA_10_12,LA_ph))
...
```{r prep_manuscript}
  ...
  qPCR_time_course<-filter(LCM.spread,!BCM %in% c("negative control")  & Study %in% c("102416","110116","110816","11217","022017_sub2","22017","22017_rerunMarch","042417","022017_run_0424","93017","52917") ) %>% select(c(Study,Sample,Replicate,BCM,ExposureTime,`miR-193b`,RNU6))
qPCR_time_course[qPCR_time_course$Study %in% c("93017","52917"),]
qPCR_time_course<-filter(qPCR_time_course, !is.na(BCM)  & !BCM %in% c("lactic acid","negative control")  & Study %in% c("102416","110116","110816","11217","022017_sub2","22017","22017_rerunMarch","11717","22017","22017_rerunMarch","042417","022017_run_0424","93017","52917")) 

qPCR_time_course$Study.comp<-factor("study_1",levels=c("study_1","study_2","study_3","LA"))
qPCR_time_course$Study.ref<-factor("study_1",levels=c("study_1","study_2","study_3","LA"))
...
qPCR_time_course<-qPCR_time_course[qPCR_time_course$BCM %in% c("L. crispatus","L. iners", "L. jensenii", "G. vaginalis", "Cell Culture Medium", "D_10","L_10","0_06_D","0_06_L","PH_766","PH_755","PH_733"),!names(qPCR_time_course) %in% c("Sample","Study")] #"0_06_D","0_06_DL","0_06_L","D_12","D_8","L_12","DL_12","DL_10","DL_8","L_8"
...
final_dir<-"/Volumes/HD-PATU3/Jacques_Steve_Shared/Manuscript/miRNA/Submission/Supplementary_Data/Analysis_Pipeline/Script_input/"
...
qpcr_sigtest<-data.frame(
  comparison=rep(c("L. crispatus","L. jensenii","L. iners"),each=2),
  reference=rep(c("G. vaginalis","Cell Culture Medium"),times=3),
ExposureTime=paste0(rep(c(0.5,1,4,8,11,13,16,19,22),each=6),"hr"),
                         Study.ref=factor("study_1",levels=c("study_1","study_2","study_5","LA")),
                         Study.comp=factor("study_1",levels=c("study_1","study_2","study_5","LA")))
...
qpcr_sigtest[qpcr_sigtest$ExposureTime %in% c("8hr","16hr","19hr") ,c("Study.ref","Study.comp")]<-"study_5"

save(qpcr_sigtest,file="/Volumes/HD-PATU3/Jacques_Steve_Shared/Manuscript/miRNA/Submission/Supplementary_Data/Analysis_Pipeline/Script_input/qpcr_sigtest.Rdata")
...
```{r LA}
<-filter(LCM.spread.summary, BCM %in% c("L_0_06","L_0_08","DL_0_06","DL_0_08")  & CellLine=="VK2") 
